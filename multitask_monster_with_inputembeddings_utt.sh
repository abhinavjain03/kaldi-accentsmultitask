...
dir=$exp/nnet3_combined_withembedding/multitask_$affix
...
dump_bnf_dir0=$exp/append_mfcc_bnf/101-recog-min
if [ $append_bnf_train -eq 1 ]; then

    steps/append_feats.sh \
        --cmd "$train_cmd" \
        --nj $nj \
      data_uttLevel/101-recog-min/cv_train_nz_sp_bnf data_uttLevel/101-recog-min/cv_train_nz_sp_hires $appended_dir0 \
      $exp/append_hires_mfcc_bnf/101-recog-min/cv_train_nz_sp $dump_bnf_dir0 || exit 1;
    steps/compute_cmvn_stats.sh $appended_dir0 \
       $exp/101-recog-min/make_cmvn_mfcc_bnf $dump_bnf_dir0 || exit 1;

fi

appended_dir1=data_uttLevel/102-cla-min/cv_trainx_nz_mfcc_bnf_appended_sp
dump_bnf_dir1=$exp/append_mfcc_bnf/102-cla-min
if [ $append_bnf_trainx -eq 1 ]; then

    steps/append_feats.sh \
        --cmd "$train_cmd" \
        --nj $nj \
      data_uttLevel/102-cla-min/cv_trainx_nz_sp_bnf data_uttLevel/102-cla-min/cv_trainx_nz_sp_hires $appended_dir1 \
      $exp/append_hires_mfcc_bnf/102-cla-min/cv_trainx_nz_sp $dump_bnf_dir1 || exit 1;
    steps/compute_cmvn_stats.sh $appended_dir1 \
        $exp/102-cla-min/make_cmvn_mfcc_bnf $dump_bnf_dir1 || exit 1;
...
if [ $decode -eq 1 ]; then
  ...
  decode_sets="cv_test_newonlyindian"
  ...
  appendForDecode=1
	...
	for x in ${decode_sets}; do
		...
		if [ $ivectorsForDecode -eq 1 ]; then			
			steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
				data_uttLevel/101-recog-min/$x $modelDirectories/nnet3/extractor ${ivec_dir} || exit 1;
		fi
    ...
    --cmd "$train_cmd" \
        ...
        $bnfNnetModelDir $exp/101-recog-min/$x $exp/make_bnf/101-recog-min

    fi

    appended_dir=data/101-recog-min/${x}_mfcc_bnf_appended
    dump_bnf_dir=$exp/append_mfcc_bnf/101-recog-min
    if [ $appendForDecode -eq 1 ]; then
      ...
      $exp/append_hires_mfcc_bnf/101-recog-min/$x $dump_bnf_dir || exit 1;
    steps/compute_cmvn_stats.sh $appended_dir \
        $exp/101-recog-min/make_cmvn_mfcc_bnf $dump_bnf_dir || exit 1;
