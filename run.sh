...
dir=exp/nnet3/multitask_$affix
...
ali_dir=exp/101-recognition/tri4_cv_train_nz_ali
if [ $align -eq 1 ]; then

  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
                   data/101-recognition/cv_train_nz_sp data/101-recognition/lang $modelDirectories/tri4 ${ali_dir}
fi
...
online_ivector_dir0=exp/101-recognition/nnet3/ivectors_cv_train_nz_sp
online_ivector_dir1=exp/102-class/nnet3/ivectors_cv_trainx_nz_sp
if [ $ivector -eq 1 ]; then
	steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
		data/102-class/cv_trainx_nz_sp $modelDirectories/nnet3/extractor $online_ivector_dir || exit 1;
fi
...
if [ $config -eq 1 ]; then
  ...
  mkdir -p $dir/configs
  num_targets0=`tree-info ${ali_dir}/tree 2>/dev/null | grep num-pdfs | awk '{print $2}'` || exit 1;
  num_targets1=16
...
if [ $egRecog -eq 1 ]; then
  ...
  steps/nnet3/get_egs.sh $egs_opts "${extra_opts[@]}" \
    ...
    --nj $nj \
      ...
      --frames-per-eg 8 \
      data/101-recognition/cv_train_nz_sp_hires ${ali_dir} $dir/egs_aligns || exit 1;

fi

ali_dir1=exp/102-class/ali
if [ $egAccents -eq 1 ]; then
  ...
  steps/nnet3/get_egs_mod.sh $egs_opts "${extra_opts[@]}" \
    --num-utts-subset 300 \
    --nj $nj \
    --num-pdfs ${num_targets1} \
      --samples-per-iter 400000 \
      --cmd "$cmd" \
      --generate-egs-scp true \
      --frames-per-eg 8 \
      data/102-class/cv_trainx_nz_sp_hires ${ali_dir1} $dir/egs_accents || exit 1;
...
if [ $train -eq 1 ]; then
  steps/nnet3/train_raw_dnn.py --stage=$train_stage \
    ...
    --trainer.srand=$srand \
    --feat-dir data/101-recognition/cv_train_nz_sp_hires \
...
if [ $priors -eq 1 ]; then
	lang_dir=$dir/101-recognition
    mkdir -p  $lang_dir
    nnet3-copy --edits="rename-node old-name=output-0 new-name=output" \
      $dir/final.raw - | \
      nnet3-am-init exp/101-recognition/tri4_cv_train_nz_ali/final.mdl - \
...
if [ $decode -eq 1 ]; then
	...
	dnn_lat_beam=8.0
	decode_stage=-2
	decode_sets="cv_dev_nz cv_test_onlynz"
	...
	for x in ${decode_sets}; do
		...
		decode=$dir/101-recognition/decode_${x}
		ivec_dir=exp/101-recognition/nnet3/ivectors_${x}
		...
		steps/nnet3/decode.sh --nj $nj --cmd "$decode_cmd" \
		      ...
		      $modelDirectories/tri4/graph_sw1_tg data/101-recognition/${x}_hires $decode | tee $decode/decode.log
	done
fi
